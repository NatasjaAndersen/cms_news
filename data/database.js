...
exports.verifyUserCreds = function(res, creds, callback){//Den slår op i db og tjekker usercreds
    var sql = "select * from users where username = ? and password = ?"
    connection.query(sql, [creds.username, creds.password], function(err, data){//Hvis usercreds passer så ryger datae over i data i argumentet
        if(err){
            helpers.respond(res, {besked : 'Database ikke tilgængelig'}, 503);
            return;
        }
...
// };
    ...
    exports.query = function(res, sql, values, callback){
        connection.query(sql, values, function(err, data){
            if(err){
                helpers.respond(res, {besked : 'Database ikke fundet'}, 503);
                return;
            }
            callback(data);
        })
